...
class Message:
    ...
    def __as_delegate(self):
        result = {'action': 'delegate', 'result': {'staking_token': None, 'staking_amount': None}}
    ...
    def __as_create_cdp(self):
        result = {'action': 'create_cdp', 'result': {'deposit_token': None, 'deposit_amount': None, 'draw_token': None, 'draw_amount': None}}
    ...
    def __as_draw_cdp(self):
        result = {'action': 'draw_cdp', 'result': {'draw_token': None, 'draw_amount': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'cdp_draw')
        if event is not None:
            amount = KavaUtil.get_attribute_value(event['attributes'], 'amount')
            amount, token = KavaUtil.split_amount(amount)
    ...
    def __as_repay_cdp(self):
        result = {'action': 'repay_cdp', 'result': {'repay_token': None, 'repay_amount': None, 'withdraw_token': None, 'withdraw_amount': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'transfer')
        if event is not None:
            ...
            result['result']['repay_amount'] = str(KavaUtil.convert_uamount_amount(amount, token))

            if len(amounts) == 2:
                amount, token = KavaUtil.split_amount(amounts[1])
                result['result']['withdraw_token'] = token
                result['result']['withdraw_amount'] = str(KavaUtil.convert_uamount_amount(amount, token))

        return result

    def __as_deposit_cdp(self):
        result = {'action': 'deposit_cdp', 'result': {'deposit_token': None, 'deposit_amount': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'transfer')
        if event is not None:
            amount = KavaUtil.get_attribute_value(event['attributes'], 'amount')
            amount, token = KavaUtil.split_amount(amount)
    ...
    def __as_withdraw_cdp(self):
        result = {'action': 'withdraw_cdp', 'result': {'withdraw_token': None, 'withdraw_amount': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'transfer')
        if event is not None:
            amount = KavaUtil.get_attribute_value(event['attributes'], 'amount')
            amount, token = KavaUtil.split_amount(amount)
    ...
    def __as_hard_withdraw(self):
        result = {'action': 'hard_withdraw', 'result': {'hard_withdraw_token': None, 'hard_withdraw_amount': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'transfer')
        if event is not None:
            amount = KavaUtil.get_attribute_value(event['attributes'], 'amount')
            amount, token = KavaUtil.split_amount(amount)
            result['result']['hard_withdraw_token'] = token
            result['result']['hard_withdraw_amount'] = str(KavaUtil.convert_uamount_amount(amount, token))

        return result

    def __as_hard_deposit(self):
        result = {'action': 'hard_deposit', 'result': {'hard_deposit_token': None, 'hard_deposit_amount': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'transfer')
        if event is not None:
            amount = KavaUtil.get_attribute_value(event['attributes'], 'amount')
            amount, token = KavaUtil.split_amount(amount)
            result['result']['hard_deposit_token'] = token
            result['result']['hard_deposit_amount'] = str(KavaUtil.convert_uamount_amount(amount, token))

        return result

    def __as_hard_borrow(self):
        result = {'action': 'hard_borrow', 'result': {'hard_borrow_token': None, 'hard_borrow_amount': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'transfer')
        if event is not None:
            amount = KavaUtil.get_attribute_value(event['attributes'], 'amount')
            amount, token = KavaUtil.split_amount(amount)
            result['result']['hard_borrow_token'] = token
            result['result']['hard_borrow_amount'] = str(KavaUtil.convert_uamount_amount(amount, token))

        return result

    def __as_hard_repay(self):
        result = {'action': 'hard_repay', 'result': {'hard_repay_token': None, 'hard_repay_amount': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'transfer')
        if event is not None:
            amount = KavaUtil.get_attribute_value(event['attributes'], 'amount')
            amount, token = KavaUtil.split_amount(amount)
            result['result']['hard_repay_token'] = token
            result['result']['hard_repay_amount'] = str(KavaUtil.convert_uamount_amount(amount, token))
    ...
    def __as_swap_exact_for_tokens(self):
        result = {'action': 'swap_exact_for_tokens', 'result': {'input_token': None, 'input_amount': None, 'output_token': None, 'output_amount': None, 'fee_token': None, 'fee_amount': None}}
        ...
        if event is not None:
            ...
            result['result']['input_amount'] = str(KavaUtil.convert_uamount_amount(input_amount, input_token))
            result['result']['output_token'] = output_token
            result['result']['output_amount'] = str(KavaUtil.convert_uamount_amount(output_amount, output_token))
            result['result']['fee_token'] = fee_token
            result['result']['fee_amount'] = str(KavaUtil.convert_uamount_amount(fee_amount, fee_token))

        return result

    def __as_swap_deposit(self):
        result = {'action': 'swap_deposit', 'result': {'share_token': None, 'share_amount': None, 'inputs': None}}
        event = KavaUtil.get_event_value(self.logs_events, 'swap_deposit')
        inputlist = []
        if event is not None:
            result['result']['share_token'] = KavaUtil.get_attribute_value(event['attributes'], 'pool_id')
    ...
    def __as_swap_withdraw(self):
        result = {'action': 'swap_withdraw', 'result': {'share_token': None, 'share_amount': None, 'outputs': None}}
    ...
    def __as_create_atomic_swap(self):
        ...
        if event is not None:
            ...
            amount, token = KavaUtil.split_amount(amount)
            result['result']['token'] = token
            result['result']['amount'] = str(KavaUtil.convert_uamount_amount(amount, token))

        return result

    def __as_claim_atomic_swap(self):
        result = {'action': 'claim_atomic_swap', 'result': {'sender': None, 'recipient': None, 'token': None, 'amount': None}}
