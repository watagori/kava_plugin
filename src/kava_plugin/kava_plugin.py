...
class KavaPlugin:
    ...
    def get_caajs(
        ...
        for message in messages:
            ...
            if result['action'] == "delegate":
                caajs.extend(KavaPlugin.__get_delegate_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "begin_redelegate":
                caajs.extend(KavaPlugin.__get_delegate_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "begin_unbonding":
                caajs.extend(KavaPlugin.__get_begin_unbonding_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "create_cdp":
                caajs.extend(KavaPlugin.__get_create_cdp_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "draw_cdp":
                caajs.extend(KavaPlugin.__get_draw_cdp_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "repay_cdp":
                caajs.extend(KavaPlugin.__get_repay_cdp_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "deposit_cdp":
                caajs.extend(KavaPlugin.__get_deposit_cdp_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "withdraw_cdp":
                caajs.extend(KavaPlugin.__get_withdraw_cdp_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "claim_usdx_minting_reward":
                caajs.extend(KavaPlugin.__get_claim_usdx_minting_reward_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "hard_withdraw":
                caajs.extend(KavaPlugin.__get_hard_withdraw_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "hard_deposit":
                caajs.extend(KavaPlugin.__get_hard_deposit_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "hard_borrow":
                caajs.extend(KavaPlugin.__get_hard_borrow_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "hard_repay":
                caajs.extend(KavaPlugin.__get_hard_repay_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "claim_hard_reward":
                caajs.extend(KavaPlugin.__get_claim_hard_reward_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == 'swap_exact_for_tokens':
                caajs.extend(KavaPlugin.__get_swap_exact_for_tokens_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "swap_deposit":
                caajs.extend(KavaPlugin.__get_swap_deposit_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "swap_withdraw":
                caajs.extend(KavaPlugin.__get_swap_withdraw_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "claim_swap_reward":
                caajs.extend(KavaPlugin.__get_claim_swap_reward_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "send":
                caajs.extend(KavaPlugin.__get_send_caajs(transaction, result['result'], token_table, address, trade_uuid))
            elif result['action'] == "create_atomic_swap" or result['action'] == "claim_atomic_swap":
                caajs.extend(KavaPlugin.__get_create_atomic_swap_caajs(transaction, result['result'], token_table, address, trade_uuid))
        ...
        if transaction_fee != 0:
            caaj_fee = KavaPlugin._get_caaj_fee(address, transaction, token_table, trade_uuid)
    ...
    def __get_delegate_caajs(cls, transaction: KavaTransaction, result, token_table, address, trade_uuid) -> list:
        ...
        for reward in result['rewards']:
            token_original_id = KavaPlugin._get_token_original_id(reward['reward_token'])
            symbol_uuid = token_table.get_symbol_uuid(KavaPlugin.chain, token_original_id)
            caajs.append(CaajJournal(
                transaction.get_timestamp(),
                cls.chain,
    ...
    def __get_repay_cdp_caajs(cls, transaction: KavaTransaction, result, token_table, address, trade_uuid) -> list:
        ...
        if result['withdraw_token'] is not None and result['withdraw_amount'] is not None:
    ...
    def __get_swap_withdraw_caajs(cls, transaction: KavaTransaction, result, token_table, address, trade_uuid) -> list:
        ...
        for output in result['outputs']:
            token_original_id = KavaPlugin._get_token_original_id(output['output_token'])
            symbol_uuid = token_table.get_symbol_uuid(KavaPlugin.chain, token_original_id)
            caajs.append(CaajJournal(
                transaction.get_timestamp(),
                cls.chain,
